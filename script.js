...
let cricbuzzOfficialApi = (url, matchId) => {
  let options = {
    method: 'GET',
    headers: {
      // 'X-RapidAPI-Key': '6de356196emsh9372a9a29f0aa3ep193c86jsnd3ede874f40f',
  ...
  .then((jsonData) => {
      ...
      localStorage.setItem("matchId"+matchId, data);
  })
  .catch(err => console.error(err));
};
...
containerEl.classList.add('container','pt-5','pb-5');
...
let createAndAppendIplMathcSectionCards = (matchDataObject) =>{
  ...
  let {date, matchId, matchDesc, matchFormat, status,  teamOne, teamTwo, venueInfo} = matchDataObject;
  let {city, ground} = venueInfo;
  let colEl = document.createElement('div');
  colEl.classList.add("col-12", "mt-1", "p-3");
  colEl.id = matchDesc;
  ...
  matchNumberEl.textContent = matchDesc +" "+ matchFormat  + ", " + "IPL 2025" + ", " + ground + ", " + city + ", India";
  // matchNumberEl.textContent = matchDesc +" "+ matchFormat + ", " + "IPL 2025";
  matchNumberEl.classList.add('match-number','mt-0', 'mb-1');
  ...
  teamInfoOneEl.classList.add('mt-2', 'ml-4'); //'team-info',
  ...
  teamInfoTwoEl.classList.add('team-info-2', 'mt-2','mr-0');
  ...
  // console.log(colEl.id);
  colEl.onclick = function() {
    // console.log(colEl.id);
    console.log(matchId);
    let id = "matchId"+matchId;
    let matchData = localStorage.getItem(id);
...
let matchCardDetails = (key, matchId, matchDesc, matchFormat,status, state,venueInfo, teamOne, teamTwo) =>{
  return{
    date:key,
    matchId,
    matchDesc,
...
let matchInfoFunction = (key, team1, team2, matchInfo, matchScore = {}) =>{
  let {matchDesc, matchId, matchFormat, status, state, venueInfo} = matchInfo;
  // console.log(matchId);
  ...
  let matchDataObject = matchCardDetails(key, matchId, matchDesc, matchFormat,status, state,venueInfo, teamOne, teamTwo);
...
let allMatchesCardData = () => {
  ...
  for(let i = 0; i < 62; i++){
    ...
    for(let j = 0; j < match.length; j++){
      ...
      // let url = "https://crickbuzz-official-apis.p.rapidapi.com/match/"+matchInfo.matchId;
      // cricbuzzOfficialApi(url, matchInfo.matchId);
...
// let ipl_2025_SeriesInfo_Object = {...ipl_2025_SeriesInfo};
// console.log(ipl_2025_SeriesInfo_Object);
